        ...
        DetailsView: FMS.FMSView.extend({
            ...
            afterRender: function() {
                ...
                if ( this.model.get('category') ) {
                    this.$('#form_category').val(this.model.get('category'));// 'Proyectos de inversión'
                }
                this.setSelectClass();
            ...
            beforeDisplay: function() {
                this.fixPageHeight();
                //this.$('#form_detail').val('Hola William~A todo el mundo~cada segundo');
                ...
                var textoDividir = detail.val();
                var detailAffectedTime = textoDividir.split('~');
                for (var j = 0; j < detailAffectedTime.length; j++)
                {
                    switch (j)
                    {
                        case 0:
                            this.$('#form_detail').val(detailAffectedTime[0]);
                        break;
                        case 1:
                            this.$('#form_affected').val(detailAffectedTime[1]);
                        break
                        case 2:
                            this.$('#form_timeaffected').val(detailAffectedTime[2]);
                        break;
                    }
                }
            },
            ...
            onClickButtonNext: function(e) {
                ...
                //if (!$('#form_affected').val()) {
                //    $('#form_affected').val() = '';
                //}

                //if (!$('#form_timeaffected').val()) {
                //    $('#form_timeaffected').val() = '';
                //}
                ...
                if ( valid ) {
                    ...
                    if ( FMS.isOffline ) {
                        ...
                        $.ajax( {
                            ...
                            type: 'POST',
                            data: {
                                category: this.model.get('category'), // 'Proyectos de inversión'
            ...
            updateCurrentReport: function() {
                ...
                this.model.set('category', category);// 'Proyectos de inversión'
                this.model.set('title', $('#form_title').val());
                this.model.set('details', $('#form_detail').val() + "~" + $('#form_affected').val() + "~" + $('#form_timeaffected').val());
